...
namespace Halfooda::BCrypt {
    ...
    char* BCrypt::encrypt(char* passowrd) {
    ...
    bool BCrypt::verify(char* password, char* hash) {
        ...
        std::cout << "Predicted: ";
        for (char i = 0; i < 31; i++) {
            std::cout << predicted_hash[i];
        }
        std::cout << std::endl;

        std::cout << "Correct:   ";
        for (char i = 0; i < 31; i++) {
            std::cout << correct_hash[i];
        }
        std::cout << std::endl;
    ...
    char* BCrypt::extract_salt(char* hash) {
        char offset = 0;
    ...
    uint32_t* BCrypt::cycle_password(char* password) {
    ...
    char* BCrypt::extract_password(char* hash) {
        char offset = 0;
        char delimiter_encountered = 0;
